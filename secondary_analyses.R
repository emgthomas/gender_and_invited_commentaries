...
journal_topics <- journal_topics[journal_topics$pub_source_title %in% unique(icc_df$pub_source_title),]
...
# define knots
n_knots <- 2 # number of *internal* knots
citescore_by_pub <- unlist(sapply(1:nrow(outputs_select),function(idx,n_cases,citescore) rep(citescore[idx],n_cases[idx]),
                                  n_cases=outputs_select$n_cases,citescore=outputs_select$citescore))
...
citescore_newmods <- seq(min(outputs_select$citescore),max(outputs_select$citescore),0.1)
...
OR_plot <- plot_ly(outputs_select, x = ~citescore, y= ~OR,height=600,width=1000) %>%
...
meta_analysis_cs_adj <- rma.mv(effect_adj,sd_adj^2,random=~1|journal,
                           mods= ~ ns(citescore,knots = knot_placement),
                           data=subset(outputs_select,!is.na(outputs_select$effect_adj)))
cat("\n\nMeta-regression on citescore\n\n")
...
OR_adj_plot <- plot_ly(outputs_select, x = ~citescore, y= ~OR_adj,height=600,width=1000) %>%
  # add horizontal line for null value
  add_trace(x = c(0,18), y= c(1,1), mode = "lines", color=I("black"),
            showlegend=F) %>%
  # add point for OR of each journal
...
# Repeat analyses for top n topics
n_topics <- 20

# Total number of cases by topic (among journals included in 2-stage meta-analysis)
topic_case_counts <- sapply(topics_list,function(topic,df) sum(df$n_cases[df[,names(df) == topic]==1]),
                            df=outputs_select)
topic_journal_counts <- colSums(outputs_select[,names(outputs_select) %in% topics_list])
...
outputs_topics <- data.frame(topic=c(as.character(topic_counts$topic[1:n_topics]),"All remaining journals"),
                             n_cases_1stage=integer(n_topics+1),
                             n_journals_1stage=integer(n_topics+1),
                             OR_1stage=numeric(n_topics+1),ci.lb_1stage=numeric(n_topics+1),ci.ub_1stage=numeric(n_topics+1),
                             OR_adj_1stage=numeric(n_topics+1),ci.lb_adj_1stage=numeric(n_topics+1),ci.ub_adj_1stage=numeric(n_topics+1),
                             n_cases_2stage=integer(n_topics+1),
                             n_journals_2stage=integer(n_topics+1),
                             OR_2stage=numeric(n_topics+1),ci.lb_2stage=numeric(n_topics+1),ci.ub_2stage=numeric(n_topics+1),
                             pi.lb_2stage=numeric(n_topics+1),pi.ub_2stage=numeric(n_topics+1),
                             OR_adj_2stage=numeric(n_topics+1),ci.lb_adj_2stage=numeric(n_topics+1),ci.ub_adj_2stage=numeric(n_topics+1),
                             pi.lb_adj_2stage=numeric(n_topics+1),pi.ub_adj_2stage=numeric(n_topics+1)
  ,stringsAsFactors = F)
...
for(i in 1:n_topics){
  ...
  # Which journals?
  journals <- journal_topics$pub_source_title[journal_topics[,names(journal_topics) == topic_counts$topic[i]]==1]
  journals_analysed <- union(journals,journals_analysed)
  icc_topic <- icc_df[icc_df$pub_source_title %in% journals,]
  outputs_topic_i <- outputs_select[outputs_select[,names(outputs_select)==topic_counts$topic[i]]==1,]
  
  # One-stage meta-analysis
  meta1 <- clogit(case ~ Gender + strata(Publication), data = icc_topic)
  ...
  meta1_adj <- clogit(case ~ Gender + seniority_quintile + strata(Publication), data = subset(icc_topic,!is.na(seniority_quintile)))
  ...
  outputs_topics$n_cases_1stage[i] <- sum(icc_topic$case)
  outputs_topics$n_journals_1stage[i] <- length(unique(icc_topic$pub_source_title))
...
# Which journals?
icc_topic <- icc_df[icc_df$pub_source_title %in% remaining_journals,]
...
outputs_topics$n_cases_1stage[n_topics+1] <- sum(icc_topic$case)
outputs_topics$n_journals_1stage[n_topics+1] <- length(unique(icc_topic$pub_source_title))
...
formatting_fun <- function(idx,or,ci.lb,ci.ub){
  paste0(sprintf(or[idx], fmt="%.2f")," (",sprintf(ci.lb[idx], fmt="%.2f"),",",sprintf(ci.ub[idx], fmt="%.2f"),")")
}

## one-stage meta-analysis
tablehead <- rbind(c("Topic","Journals","Cases","OR (95%CI)","AOR (95%CI)"),
                   rep(NA,5)
                   )
tablenum <- cbind(as.matrix(outputs_topics[,c("topic","n_journals_1stage","n_cases_1stage")]),
...
forestplot(tabletext,mean=means,lower=lowers,upper=uppers,
           ...
           legend = c("1-stage unadjusted", "1-stage adjusted"),
...
tablenum <- cbind(as.matrix(outputs_topics[,c("topic","n_journals_2stage","n_cases_2stage")]),
                  sapply(1:nrow(outputs_topics),formatting_fun,
...
# make plot/table
pdf(file="./results/forestplot_2stage.pdf",width=11,height=8)
forestplot(tabletext,mean=means,lower=lowers,upper=uppers,
           is.summary=c(TRUE,TRUE,rep(FALSE,nrow(outputs_topics)),TRUE),
           col=fpColors(box=c(cols1[5], cols2[5]),summary=c(cols1[5], cols2[5])),
           zero=1,
           ...
           fn.ci_norm = c(fpDrawDiamondCI, fpDrawCircleCI),
           #fn.ci_sum=c(fpDrawDiamondCI, fpDrawCircleCI),
           legend = c("2-stage unadjusted", "2-stage adjusted"),
           legend_args = fpLegend(pos = list("topright")),
           new_page=F
)
dev.off()
